...
export const CodeComparison: React.FC<CodeComparisonProps> = ({ originalCode, fixedCode }) => {
  ...
  // Simple line change detection (basic implementation)
  const isLineChanged = (lineIndex: number): { left: boolean; right: boolean } => {
    const originalLine = originalLines[lineIndex]?.trim() || '';
    const fixedLine = fixedLines[lineIndex]?.trim() || '';
  ...
  const renderCodePanel = (lines: string[], side: 'left' | 'right') => {
    return (
      <div 
        className={`flex-1 overflow-auto max-h-96 ${isExpanded ? 'max-h-none' : ''}`}
          ...
          <code>
            {Array.from({ length: maxLines }, (_, i) => {
              ...
              return (
                <div 
                  key={i} 
                  className={`flex ${bgColor} ${isHighlighted ? 'px-2 -mx-2' : ''}`}
                >
    ...
    <div className="space-y-4">
      <Card className="overflow-hidden">
        <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-200">
          ...
          <div className="bg-gray-900 text-gray-100">
            <div className="bg-red-900/30 px-4 py-2 border-b border-red-800/50">
          ...
          <div className="bg-gray-900 text-gray-100">
            <div className="bg-green-900/30 px-4 py-2 border-b border-green-800/50">
              <h4 className="font-semibold text-green-100 flex items-center gap-2">
      ...
      <div className="flex flex-wrap gap-4 text-sm text-muted-foreground justify-center">
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 bg-red-100 border-l-2 border-red-300 rounded-sm"></div>
          <span>Removed/Problematic lines</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 bg-green-100 border-l-2 border-green-300 rounded-sm"></div>
